...
## 06. 세트=식약처 신고 제품 재구성 + 카드 정보 페르소나 정렬 (2026-05-30)

- **요구사항**: ① 페르소나 "세트 구성"을 식약처 신고(크롤) 제품으로 교체 ② 카드에 노출되는 정보가 정말 페르소나가 궁금한 것인지 점검.
- **고민**: #05에서 가격·할인율을 카드 전면에 세웠으나, 페르소나 리서치(P3)상 "가격비교"는 네이버·쿠팡이 이미 하는·페인 미해결 영역이고 `listPrice`는 코드상 "데모 더미"다. 정작 중복·과다를 좌우하는 **성분·함량은 카드에 0**. 세트를 크롤 제품으로 바꾸면 **크롤 의존성**(미실행 시 이름 미해석→카트 빔)이 생김.
- **해결**: 세트 3종을 식약처 신고 제품으로 재구성하고 `/analyze`로 의도 판정 검증(care 만8세 비타민D 4중 겹침→**OVER**, working 만6세 단일→**SAFE**, beginner 만3세 유산균→**UNKNOWN**). `ProductCard`에서 정상가 취소선·할인율 theater 제거 → **핵심 성분·함량(상위 3종)** 전면 + 식약처 인증 **씰(seal)**(`MfdsSeal`, sourceUrl `식약처…신고번호 N` 파싱), 가격은 작게 강등. 크롤 의존성은 `personaScenarios.ts` 주석에 명시. lint/type 그린(기존 Clerk/vite 2건 제외). MFDS 크롤은 `MFDS_SERVICE_KEY` 설정 후 "어린이" 151건 중 파싱 74건→캐논8종 매핑 49건 seed.

## 07. 세트 섹션 제목을 엄마 관점 설득 헤드라인으로 (2026-05-30)

- **요구사항**: "상황별 추천 세트"의 각 세트 제목이 페르소나(엄마) 관점에서 설득되어야 함 — 예: "환절기 면역이 걱정되는 엄마들 주목!".
- **고민**: 기존 `setName`은 상품 라벨톤("환절기 면역 케어 세트")이라 후킹이 약함. `setName`을 그대로 긴 설득 문구로 바꾸면 히어로 카드의 "상품 세트 라벨" 역할(`SetSection` 오버레이)이 망가짐 — 제목(후킹)과 카드 라벨(커머스)은 역할이 다름.
- **해결**: `PersonaScenario`에 `headline?`(엄마 관점 설득 헤드라인) 필드 추가. care/working/beginner 각 페인을 직접 찌르는 문구로 작성("환절기 면역 챙기는 엄마들 주목!…", "시간 없는 워킹맘 주목!…", "첫 영양제…막막한 초보맘 주목!"). `SetSection` h2만 `headline ?? setName`으로 렌더(폴백 유지), 히어로 오버레이는 `setName` 그대로 → 후킹/라벨 분리. 타입체크 그린(기존 Clerk/vite 2건 제외).

## 08. 결제 넛지 모달 2단계화 — 나이 확인 → 간단 분석 (2026-05-30)

- **요구사항**: `결제하기` 누르면 모달이 ① "혹시 만 N세 맞나요?" 나이를 먼저 확인하고 → ② 간단하게 "AI가 분석해 드리고 적정한지·얼마나 먹어야 하는지" 보여주는 흐름. (기존: 나이를 checkout 페이지에서 받고, 모달은 상세 ReportCard로 빽빽)
- **고민**: ① 나이 입력 위치(checkout 잔류 vs 모달 이동) ② "간단함"과 "근거(신뢰)"의 충돌 — 성분별 상한·근거·LLM설명을 다 빼면 신뢰 신호가 사라짐 ③ "AI 종합 문단"을 새 LLM 호출로 만들지(비용·환각 가드) vs 룰 판정으로 조합할지 ④ 성분 많을 때 1줄 리스트 길이.
- **해결**: 나이를 **모달 STEP1**로 이동(checkout 입력칸 제거, 페르소나 prefill은 STEP1 초기값). 결과는 **간단 요약 + '자세히 보기'**: 종합 배너 + **룰 판정으로 클라가 조합한 🤖 문단**(`composeSummary`, LLM 추가호출 0·환각 불가) + 성분별 1줄 전부(`formatNutrientLine`, `VERDICT_STYLE` 공유) + 토글 시 기존 `ReportCard`(상한·근거·실제 LLM설명) → 간단함·신뢰 양립. 위험 시 `RecommendedSetCard`(OVER 시 자동 노출). 백엔드 무변경(`/analyze` 재사용). 헬퍼는 TDD(vitest unit 5케이스), 단계별 spec/품질 서브에이전트 리뷰. 검증: 안전·위험 e2e(localhost, 배포 API는 price 없는 구버전이라 로컬 API 사용) 통과, API 60 테스트 회귀 0. 별도 `chore`: `vite`를 web devDep로 명시해 vitest 정상화(프로젝트 전반 미동작이던 것 수정).
## 09. 성분 돋보기(NutrientInfo) — 나이별 적정량·단위 팝오버 (2026-05-30)
